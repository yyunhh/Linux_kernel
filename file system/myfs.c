...
{
        struct dentry *input_d;
        struct dentry *output_d;
 
        atomic_set(&a, 0);
        atomic_set(&b, 0);

        input_d = myfs_create_dir(sb, root, "input");
        output_d = myfs_create_dir(sb, root, "output");
        
        if(input_d){
                myfs_create_file(sb, input_d, "a", &a);
                myfs_create_file(sb, input_d, "b", &b);
        }else{
        ...
        if(output_d){
                myfs_create_file(sb, output_d, "add", &add);
                myfs_create_file(sb, output_d, "sub", &sub);
        }else{
